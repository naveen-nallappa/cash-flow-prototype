/* Print-formatted Multi-Generational Family — 8.5 x 11, 5 pages */
...
function PrintMultiGen() {
  ...
  return (
    <>
      {/* PAGE 1 — Cover & KPIs */}
      <div className="page">
        <PrintRunners left="MERIDIAN · CONSOLIDATED CASH FLOW" right="PAGE 01 / 05" />
        ...
        <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:16, marginTop:16, borderTop:'1px solid var(--rule-strong)', paddingTop:12}}>
          <div><div style={{fontSize:'7pt', letterSpacing:'0.18em', textTransform:'uppercase', color:'var(--ink-500)'}}>Reporting Entities</div><div style={{fontSize:'10pt', fontWeight:500, marginTop:2}}>6 entities · 14 accounts</div></div>
          <div><div style={{fontSize:'7pt', letterSpacing:'0.18em', textTransform:'uppercase', color:'var(--ink-500)'}}>Net Worth (Est.)</div><div style={{fontSize:'10pt', fontWeight:500, marginTop:2}}>$420M consolidated</div></div>
          <div><div style={{fontSize:'7pt', letterSpacing:'0.18em', textTransform:'uppercase', color:'var(--ink-500)'}}>Generations Active</div><div style={{fontSize:'10pt', fontWeight:500, marginTop:2}}>G1 (2) · G2 (3) · G3 (4)</div></div>
          <div><div style={{fontSize:'7pt', letterSpacing:'0.18em', textTransform:'uppercase', color:'var(--ink-500)'}}>Lead Advisor</div><div style={{fontSize:'10pt', fontWeight:500, marginTop:2}}>D. Yates · K. Park, Trust Counsel</div></div>
        </div>

        <div className="p-kpi-strip" style={{marginTop:14}}>
          <div className="p-kpi"><span className="label">Consolidated Inflows</span><span className="num">{fmt.m(totalIn)}</span><span className="delta pos">▲ +4.8%</span></div>
          <div className="p-kpi"><span className="label">Consolidated Outflows</span><span className="num">{fmt.m(totalOut)}</span><span className="delta neg">▲ +6.1%</span></div>
          <div className="p-kpi"><span className="label">Net Movement</span><span className="num neg">({fmt.m(Math.abs(totalNet))})</span><span className="delta">funded by reserves</span></div>
          <div className="p-kpi"><span className="label">Family Distributions</span><span className="num" style={{color:'var(--gold-700)'}}>$6.40M</span><span className="delta">G2 (×3), G3 (×4)</span></div>
        </div>
      ...
      {/* PAGE 2 — Monthly + Sources */}
      <div className="page">
        <PrintRunners left="MERIDIAN · CONSOLIDATED CASH FLOW" right="PAGE 02 / 05 · MONTHLY FLOWS" />
        <PrintMast archetype={archetype} period={period} />
        ...
        <div className="p-frame">
          <StackedMonthlyChart months={months} sources={inflowSources} height={200} />
      ...
      {/* PAGE 3 — Outflows */}
      <div className="page">
        <PrintRunners left="MERIDIAN · CONSOLIDATED CASH FLOW" right="PAGE 03 / 05 · OUTFLOWS" />
        <PrintMast archetype={archetype} period={period} />
        ...
        <div className="p-frame" style={{display:'grid', gridTemplateColumns:'auto 1fr', gap:20, alignItems:'center'}}>
          <Donut size={170} data={inflowSources.map(s => ({ name: s.name, color: s.color, value: s.values.reduce((a,b)=>a+b,0) }))} />
        ...
        <div className="p-section-head">
          <div><span className="eyebrow">EXHIBIT 05</span><h2>Outflows by Category</h2></div>
          <div className="right">FY 2025</div>
        </div>
        <div className="p-frame">
      ...
      {/* PAGE 4 — Tax + Debt */}
      <div className="page">
        <PrintRunners left="MERIDIAN · CONSOLIDATED CASH FLOW" right="PAGE 04 / 05 · TAX & DEBT" />
        <PrintMast archetype={archetype} period={period} />
      ...
      {/* PAGE 5 — Philanthropy + Forecast */}
      <div className="page">
        <PrintRunners left="MERIDIAN · CONSOLIDATED CASH FLOW" right="PAGE 05 / 05 · GIVING & FORECAST" />
        <PrintMast archetype={archetype} period={period} />

        <div className="p-section-head">
          <div><span className="eyebrow">EXHIBIT 08</span><h2>Philanthropic Giving · Foundation + Direct</h2></div>
          <div className="right">FY 2025</div>
        </div>
        <div className="p-frame">
